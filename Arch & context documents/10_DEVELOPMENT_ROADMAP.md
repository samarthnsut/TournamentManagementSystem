...
**Deliverables**
- [x] Migrations: `audit_log` (V12), `document` + `document_upload` (V13)
- [x] `Document` module: presigned S3 upload/download, `{ organizationUnitId, entityType, entityId, fileName, fileUrl, mimeType, sizeBytes, uploadedBy }`; mime/size allow-list enforced at init *and* re-checked against the stored object at attach; attach to Registration and Tournament. Proved against MinIO in Testcontainers, not a mock
